...
0. **공용 플레이북 — 전문 대신 내 뷰만 읽는다** ★[9/9 토큰 절감]:
   ```
   python3 .claude/skills/portfolio-desk/scripts/playbook.py --desk research-feed
   ```
...
3b. **🎞️ [8/22 상시 운용 · 정훈 승인] 화면 교차검증 — 롱폼 한정.**
   `python3 .claude/skills/portfolio-desk/scripts/yt_frames.py <영상ID> --sheets 2`
   → 스토리보드 시트 경로 출력 → **Read로 열어 판독**. (스토리보드 = 실제 영상 프레임 격자.
   yt-dlp·재생 캡처는 이 환경에서 전부 막혔고 이 경로만 뚫렸다 — 8/22 실측.)

   **⚠️ 쇼츠는 하지 말 것 — 도구가 자동 경고한다.** 쇼츠(세로)는 L2 타일이 **50×90**으로
   쪼그라들어 숫자·문구가 안 읽힌다(8/22 실측 2편 판독 실패). **롱폼(160×90)만 유효**하며,
   경제사냥꾼 일일 업로드는 **상당수가 쇼츠**다. 쇼츠 경고가 뜨면 **즉시 중단하고 자막으로 갈음**한다.

   - **표적(우선순위 순)** — 전량 순회 금지(latest 126편·아카이브 597편):
     ① **[정정] 이력이 있는 롱폼** — 채널이 틀렸던 건 화면에 원자료가 떠 있는 경우가 많다.
        ⚠️ 8/22 첫 실증에서 잡힌 건 **채널 오류가 아니라 우리 전사 오차**였다
        (삼성 영업활동현금흐름: 우리 로그 105.1조 ↔ 화면 뉴스 원문 **105조 8,800억**).
        **화면 검증은 채널만 검사하는 게 아니라 우리 요약도 검사한다.**
     ② **구체 수치·목표가·차트를 주장하는 롱폼**인데 웹서치 교차검증이 갈리거나 안 잡힐 때.
     ③ 자막 확보가 끝내 실패한 롱폼 → 주제·종목만 파악(**분석 금지**, "화면만 확인"으로 표기).
   - **상한: 1회 실행당 롱폼 최대 2편 · 편당 2시트.** 시트 1장 ≈1,100~1,600토큰.
     상한을 넘길 사유가 생기면 PM에게 보고하고 진행.
   - ⚠️ **읽히는 것 / 안 읽히는 것**(롱폼 기준): 종목 화면의 가격·등락률, 지수 화면, 뉴스 원문
     문장, 큰 자막까지 읽힌다. **차트 축 눈금·표 세부 숫자는 안 읽힌다** — 못 읽은 걸 읽은 척하지
     말고 "화면 확인 불가"로 쓴다. 화면에서 읽은 수치도 **1차 출처와 교차**한 뒤 [검증] 승격.
   - 화면을 본 영상은 리턴 포맷에 **[화면확인]** 표기 + 무엇을 봤는지 한 줄.
     **우리 로그·요약과 어긋나면 그 사실을 반드시 리턴에 넣는다**(위 ① 사례가 그렇게 잡혔다).
...
**① "{영상 제목}" ({업로드일}, {길이})**
- 핵심 주장: {…}
- {주장1} → [검증/정정/**대립·미결**/미확인] ({근거})
- {주장2} → [검증/정정/대립·미결/미확인]
  ※ ★[8/27 신설] **[대립·미결]** = 채널 주장 vs 다른 전망이 맞서는데 **결과가 아직 안 나온 것**. 설문·컨센서스를 근거로 [정정]을 붙이면 *컨센서스를 사실로 취급*하는 오류다 — 8/27 금통위에서 실제로 났고 **채널이 맞았다**. [정정]은 결과가 확정된 뒤에만 붙인다.
  ※ ★[8/27 신설] **[미확인]은 반드시 사유를 괄호로 적는다** — `[미확인(불가:유료)]` / `[미확인(미시도)]` / `[미확인(결과대기)]` 3종. 아카이브 500건 중 미확인이 85건 남은 원인은 게으름이 아니라 **분류 부재**였다: ⓐ확인 불가(유료·라이브) ⓑ확인 안 함 ⓒ아직 결과 없음이 한 바구니에 있으면 ⓒ가 ⓑ처럼 보여 아무도 판별 못 한다. **사유가 `미시도`인 것만이 6/28 정훈 지시('미확인 무조건 다 확인')의 대상**이고, `결과대기`는 [대립·미결]로 승격 후보다. (8/12 '지표의 분모를 의심하라'의 연장 — 셋을 합산하면 '확인율'이 지표로서 무의미해진다.)
- 코멘트: {정훈 포트폴리오·룰과의 연결}
