...
매번 똑같은 AI 기본 룩(보라색 그라데이션·의미 없는 3색 팔레트·과한 그림자)을 피하고, **금융 차트 관습**을 따른다.
- **의미 있는 색만**: 순매수=초록 / 순매도=빨강 / 지수 라인=중립 단색(남색·회색). 장식용 컬러 금지.
- **데이터 잉크 우선**: 불필요한 배경·범례·격자 최소화, 값 라벨은 막대마다 직접. 한 차트에 한 메시지.
- **정훈이 폰으로 본다**: 글자·라벨 충분히 크게, 핵심 수치(연속일수·누적액·돌파선)는 주석으로 콕.
- 화려함보다 **"한눈에 신호가 읽히는가"**가 기준. 애매하면 단순화.
