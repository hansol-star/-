...
2b. **📱 앱 데이터 갱신 (매 보고서 필수)** — 종목별 분석 정본 `data/app/stocks.json`의 해당 종목 `stars`·`target`·`buy_zone`·`trim`·`comment`·`issues`(최신 [검증/정정/미확인] 이슈를 날짜와 함께 prepend)·`as_of`·`source_report`를 이번 보고서값으로 갱신한 뒤, 시세를 합쳐 앱 데이터를 빌드한다:
   ```bash
   python3 .claude/skills/portfolio-desk/scripts/build_app_data.py   # → app/data.js 생성
   ```
   이걸로 정훈의 포트폴리오 앱(`app/index.html`)이 최신 시세·손익·목표·이슈로 갱신된다. (네트워크 차단 시 `--offline`.)
3. **git 커밋**(연속성의 백본 — 구버전은 git 히스토리에 남으므로 archive 폴더 불필요):
   ```bash
   git add docs/ data/app/stocks.json app/data.js && git commit -m "보고서 v{N} {날짜}: {한줄요약}"
   ```
