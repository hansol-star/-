...
**📖 [7/4 · ★9/9 개정] 공용 플레이북 — 전문 대신 데스크별 뷰**: 전 데스크(9개) agent 파일 Tasks 0 =
`python3 .claude/skills/portfolio-desk/scripts/playbook.py --desk <이름>`.
**왜**: 전문은 31,994자인데 데스크가 실제로 쓰는 건 **§1 공통(4,608) + 자기 §3(평균 1,450)**뿐이다.
나머지 26,000자(§2 캘리브레이션 11,660 + 남의 데스크 §3 약 13,000)를 9개가 각자 읽고 있었다 —
**전문 9회 287,946자 → 뷰 9개 96,591자 = 66% 절감**(실측).
⚠️ **정본은 여전히 `docs/desk_playbook.md`** — 교훈 append는 원문에 한다(§4 지속 학습 루프).
⚠️ §2는 **최신 2건 전문 + 과거 헤드라인**으로 남긴다(통째로 버리면 같은 편향을 반복한다).
소스 우선순위(6/16) 정본도 이 파일 §1. PM도 종합 전 §2·§3 확인.
...
**★[9/9 기계화] 섹터 데스크 스폰은 먼저 게이트를 돌려 판정한다 — 습관으로 전부 띄우지 말 것:**
```bash
python3 .claude/skills/portfolio-desk/scripts/desk_gate.py
```
**왜 생겼나**: 아래 트리거 규칙이 **문장으로만** 있었고 **아무도 계산하지 않았다.** 9/9에 PM이
"주간 첫 보고서니까"로 7개를 전부 띄웠고, 세션 한도로 4개가 죽어 **두 번** 띄웠다 —
데스크 하나가 평균 **13만 토큰**(실측: risk 155,499·kr 136,681·us 128,804)이라 그날만 150만 토큰이다.
8/2 *"오더북에 들어간 것만 집행된다"*의 재현 — **계산되지 않는 게이트는 게이트가 아니다.**
⚠️ 게이트는 **권고**다. 테마 뉴스·정훈 지목은 기계가 모르므로 PM이 덮을 수 있다 —
   단 **출력을 보고 의식적으로** 덮을 것(습관적 전체 호출 금지).
...
### 4b. 실현손익·통화·리스크 3종 (매 보고서 필수) [8/23 신설]
평가손익만 보던 축을 셋 더 붙인다 — **전부 측정 전용, 어떤 룰도 바꾸지 않는다.**
```bash
...
- **① 원장 대사가 먼저다.** `--reconcile`이 실패하면 **보고서를 계속 쓰기 전에 원인부터 잡는다** —
  ...
  체결이 있었으면 `trades.py --add ... --source "<스크린샷/보고서>"`로 원장에 먼저 넣고 portfolio.json을 맞춘다.
- **② 실현손익을 STATE SNAPSHOT과 §7 PM 종합에 병기**한다. 평가손익만 쓰면 **이미 확정된 성과가 보고서에서 사라진다**
  (roadmap §1: 산출물의 유일한 평가자는 정훈의 실제 계좌 수익률).
...
2b. **📱 앱 데이터 갱신 (매 보고서 필수 — ⚠️[2026-07-02] EXEC·밤 대화·부록 세션도 예외 없음)** — 아래 정본들을 이번 보고서값으로 갱신. **[7/2 사고 재발 방지]**: v37 아침 서사(코스피 8,303 "존 근접")가 EXEC 폭락(7,648) 뒤에도 stocks.json에 남아 앱이 시세=폭락/서사=아침 분열 상태로 노출됐음 → `_exec`/`_night` 부록 세션도 **stocks(as_of·source_report·forecast·buy_zone·comment)·hunter·tasks·flows 4파일을 반드시 그 세션 상태로 동기화**하고 build 재실행. validate_report.py가 source_report **파일명 전체**를 최신 보고서 파일과 대조해 intra-version stale을 FAIL로 적발한다:
   - `data/app/stocks.json`: 종목별 `stars`·**`score`(0~100 정량점수, self-review가 후행검증)**·`target`·`buy_zone`·`trim`·`comment`·`issues`(최신 [검증/정정/미확인] 이슈 날짜와 함께 prepend)·`as_of`·`source_report`.
     ★[9/22] **워치(`watchlist`)도 같은 의무다** — 보고서 워치 표에 쓴 새 목표·매수존·재료를 되쓰고 **`prose_as_of`를 그날로** 찍는다. as_of·price는 자동 갱신돼 겉보기엔 신선하므로 날짜 기준은 산문 날짜다(9/22: 워치 11종이 8/19~21 산문 그대로 한 달 — 폐기 룰 '안전핀 해제'가 AMD 매수 조건으로, 다른 종목 가격이 한화오션 코멘트로 떠 있었다). `check_watch_prose`가 14일 초과·폐기 룰 현행 서술을 WARN.
