...
보고서를 저장·커밋하기 전에 자가검증한다 (새 스크립트 없이 이미 돌린 결과 재활용):
- pnl.py / triggers.py / consensus.py 출력과 보고서 표 숫자가 일치하는지 대조.
- 단일출처·미검증 수치 → "미확인" 표기 확인.
- 워치리스트 티커 불일치(원익IPS·테스) 인용 시 Yahoo 시세 재검증.
- 직전 STATE SNAPSHOT 대비 모순 없는지 확인.
통과 못 하면 수정 후 커밋. (검증 루프 = 결과 품질 2~3배 — 인스타 팁5.)
