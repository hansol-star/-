...
0. **⚠️ 버전 충돌 방지 [2026-06-17 추가]**: 다른 세션이 같은 날/번호로 보고서를 만들었을 수 있다 → **먼저 원격 최신을 가져와 확인.**
   ...
   ```
   로컬보다 origin/main이 앞서면 `git merge --ff-only origin/main`(또는 rebase)로 정합 후 시작. **새 보고서 번호 = origin 최신 버전 +1**(같은 날이라도 번호 겹치면 안 됨). 푸시는 main 머지가 기본(연속성 규약).
...
- **index_forecast**: 코스피·나스닥·S&P·필반 예상 레인지(`ref`현재·`low`·`base`·`high`·`dir`·`note`).
...
2b. **📱 앱 데이터 갱신 (매 보고서 필수)** — 아래 3개 정본을 이번 보고서값으로 갱신:
...
- **자가 콜 검증 [self-review 스킬]**: **주간 첫 보고서**(또는 큰 장세 변화 뒤)에는 `self-review`를 돌려 과거 별점·스코어·목표가·매수존 콜이 실제로 맞았는지 후행검증하고 `docs/research/call_scorecard.md`에 누적·캘리브레이션. 편향(별점 쏠림·순서 역전)·반복 빗나감은 채점기준/전망 교정을 정훈에게 제안(자동 변경 ❌). 평일 보고서는 생략 가능.
