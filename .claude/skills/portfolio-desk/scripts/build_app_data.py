...
def hunter_scorecard(verdicts) -> dict:
    ...
    for v in verdicts:
        ...
        if not v or "미채점" in v:
            ...
            b["미채점"] += 1
        elif "일반" in v or "방향성 서술" in v or "참고·방향" in v:
            # ★[8/12] '방향성 서술'·'참고·방향' = **보긴 봤는데 검증 가능한 수치 콜이 없는** 것.
            # '미채점'(아직 안 봄)과 다르고 '미확인'(확인 실패)과도 다르다 → 교육·원칙론과 같은
            # '채점 대상 아님' 취급. 舊 로직은 이걸 else로 흘려보내 미채점으로 오분류했다(5건).
            b["일반"] += 1
