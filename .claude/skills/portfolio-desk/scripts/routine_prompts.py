...
VARIANT = {
    ...
    "r4b": ("\n- 이 세션은 R4b(21:15)다. 완주를 우선으로 두되(품질 < 완주), **또 막히면 억지 축약본 대신 "
            "그대로 종료**한다 — 다음날 02:30 R4c가 리셋된 창에서 이어받는다. "
            "정훈 폰창(20:50) 밖이라 집행은 다음 폰창·예약주문으로 넘긴다."),
    "r4c": ("\n- 이 세션은 **R4c(익일 02:30)** 다 — [9/9 신설]. R4a·R4b가 둘 다 토큰에 막힌 날의 "
            "**진짜 마지막 기회**이고, 실측된 세션 한도 리셋(02:00 KST) 직후 창이라 예산이 가장 신선하다.\n"
            "- ⚠️ **보고서 기준일은 '어제 거래일'이다.** 지금은 자정을 넘겼지만 이 보고서가 다루는 것은 "
            "**전 거래일 종가**다 — `kst_now.py`로 오늘을 확인하되 파일명·본문 기준일은 그 전 거래일로 잡고, "
            "그 사실을 보고서 머리에 한 줄로 명시한다(7/6 날짜 앵커링 규율의 확장).\n"
            "- 완주를 최우선으로 둔다(품질 < 완주). 집행 액션은 전부 다음 폰창·예약주문으로 넘긴다 — "
            "새벽엔 정훈이 없다."),
}
