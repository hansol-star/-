...
def score(rows: list[dict]):
    ...
    # ② 폭풍 감산 유효성
    #    ⚠️ 금액 감산은 **2026-07-30 2차 개정으로 폐지**됐다(해금 4단계 전부 역효과 확인).
    #    아래는 그 판정의 근거를 재현하는 **이력 검증**이며, 현행 룰에는 감산이 없다.
    #    (백테스트 표본은 舊 감산 계단으로 생성되므로 여전히 버킷이 갈린다.)
    print("\n  ② 폭풍 감산 [폐지된 조항 · 이력 검증] — 감산이 강했던 날이 실제로 나빴나")
