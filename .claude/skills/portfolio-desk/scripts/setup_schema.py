...
def calc_met_pct(conds) -> int:
    ...
    ★[2026-09-21 정훈 승인 d202 — 부호 섞임 수정]
    舊: 모든 조건을 동등하게 셌다. 그래서 `setup-엔캐리청산경계-0803`의 조건③
    *"외인 4일 연속 순매수 지속(청산압력 **소멸** 확인)"* 처럼 **경계를 내리는 신호**가
    "2개+ 충족 시 경계 **격상**"이라는 action에 그대로 가산됐다 —
    **충족될수록 경계가 올라가는 역방향**이다. 9/20에 ③이 false여서 판정엔 영향이 없었지만,
    ③이 켜지는 날 조용히 틀린 경보가 난다.
    新: `role: "release"`인 조건은 분모·분자 **둘 다에서 제외**한다(해제 조건은 따로 읽는다).
    role 미지정 = 격상 조건(기존 셋업 전부 그대로 동작 — 하위호환).

    ⚠️ 왜 "부정형으로 뒤집기"를 안 골랐나: 조건③을 *"4일 연속 순매수 **미달**"* 로 뒤집으면
    실측 기저율이 **95.5%**(66거래일 중 63일)라 거의 항상 참인 조건이 된다 —
    셋업이 영구 3/3에 고정되고 신호가 죽는다. **드물게 켜지는 쪽(4.5%)을 해제 신호로 쓰는 게 맞다.**
    """
    conds = [c for c in (conds or []) if (c or {}).get("role") != "release"]
    if not conds:
        return 0
