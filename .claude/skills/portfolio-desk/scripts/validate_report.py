...
def check_kr_price_band():
    ...
    **애초에 오더북에 들어갈 수 있는 가격일 것**.
    ...
    for o in d.get("orders", []) or []:
        ...
        # ★[9/3 신설 — 시행일 조건부] 2026-09-14부터 **시간외단일가 제도가 폐지**된다.
        #   舊: 시간외단일가 16:00~18:00 · 10분 단위 일괄체결 · **당일 종가 ±10%**
        #   新: 시간외접속매매(애프터마켓) 16:00~20:00 · **정규장과 동일 실시간 체결**
        #       + 프리마켓 07:00~07:50 신설 → 전체 거래시간 07:00~20:00
        #       가격제한폭 = **전일 종가 ±30%로 정규장과 동일** · **지정가 주문만**
        #       제외: ETF·ETN, 당일 정규장 미거래, 투자경고/위험, 이상급등, 관리종목, 초저유동
        #   출처: 한국거래소 — 서울신문·다음(3/21 확정)·이투데이·베타뉴스(9/1) 다출처 [검증]
        #
        # ⚠️ **오늘부터 바꾸면 안 된다.** 9/13까지는 舊 ±10%가 실제 제약이고,
        #   그때 이 검사를 ±30%로 열면 접수 거부될 오더를 통과시킨다(8/12 LG전자 사고의 재발).
        #   반대로 9/14 이후에도 ±10%를 유지하면 **멀쩡한 오더를 막는다**.
        #   ⇒ 시행일로 갈라 자동 전환한다. 날짜가 지나면 이 분기는 죽은 코드가 되지만
        #     **언제 무엇이 바뀌었는지의 기록**으로 남긴다(폐기룰 잔존과 다르다 — 이건 이력이다).
        AFTER_MKT_REFORM = dt.date(2026, 9, 14)
        _today_kst = (dt.datetime.utcnow() + dt.timedelta(hours=9)).date()
        if _today_kst >= AFTER_MKT_REFORM:
            # 애프터마켓이 정규장과 같은 밴드를 쓰므로 별도 경고가 불필요하다.
            # 남는 제약은 '지정가만 가능'과 제외종목인데, 우리 국내 오더는 전부 지정가이고
            # 보유 5종목은 제외 대상이 아니다(ETF·관리종목 없음).
            pass
        else:
            ...
            if floor <= price <= cap and not (aft_floor <= price <= aft_cap):
                warn(f"시간외 즉시주문 불가 [{oid}]: 지정가 {price:,.0f}원이 시간외단일가 밴드 "
                     ...
                     f"※ 9/14부터 이 제약은 사라진다(시간외접속매매 = 정규장과 동일 ±30%)")
...
def check_low_star_action():
    ...
    ★[8/29 정훈 승인 — 방향 권장을 철회하고 중립화] 舊 문구는 8/22 실측을 근거로
      *"⭐1은 트림이 맞다(알파 -8.75%) / ⭐2는 팔면 손해였다(+3.31%)"*고 **방향을 권장**했다.
      8/29 재검정에서 그 두 숫자가 **단일 종목의 산물**임이 확인됐다:
        · ⭐1 n=12 = **두산로보 한 종목의 12일치**. 종목 1개는 통계가 아니다.
        · ⭐2 n=130 = 5종목이지만 **NAVER(+22.2%·n33)가 버킷을 지배**한다.
          `star_validate` LOO에서 NAVER를 빼면 ⭐5-⭐2 격차가 -7.60→-0.99%p로 소멸한다.
      같은 날 같은 오염 표본이 `STAR_PROB` 재보정에도 박혀 ⭐2>⭐3 **비단조**를 만들었다
      (8/29 단조 복원 + check_star_prob_monotonic 가드 신설).
    ⇒ **결정 강제(이 FAIL)는 유지하되 방향 권장은 제거한다.** ⭐2 이하가 '관망'으로
      넘어가지 못하게 막는 근거는 알파 통계가 아니라 **8/2 실측**이다
      (7주 손실의 68.7%가 ⭐2 두 종목 · 산문 트림 8~9회 vs orders 0회) — 그건 그대로 유효하다.
    ⚠️ "무엇을 할지"는 표본이 답하지 못한다. **트림이냐 홀드냐는 PM이 종목별로 판단**하고,
      이 가드는 **판단을 했는지만** 본다."""
    ...
    for t, v in (st.get("stocks") or {}).items():
        ...
        if not has_order and not decided:
            # [8/29] 방향 권장 철회 — 근거였던 두 알파가 단일 종목의 산물이었다.
            rec = ("트림 오더 또는 '홀드 + 재검토 트리거·기한' 중 하나를 명시 "
                   "(⚠️ 어느 쪽이 유리한지는 표본이 답하지 못한다 — ⭐1 n=12는 두산로보 1종목, "
                   "⭐2 알파는 NAVER 1종목이 지배. 방향은 종목별 PM 판단)")
