...
python3 .claude/skills/portfolio-desk/scripts/watch_calls.py --save       # ★[9/14] 워치 콜 원장화 — 보고서 워치 풀표의 ⭐를 회수. **star_validate보다 먼저** 돌 것(안 돌면 옛 표본으로 채점한다)
...
python3 .claude/skills/portfolio-desk/scripts/star_validate.py --with-watch --beta-adj  # ★[9/14] 같은 검정을 **표본 37종목 + 젠센 알파**로 — 위 '보유만·단순차분'과 **나란히** 읽는다(둘이 갈리면 그게 산출물)
...
- **손익비·기대값** [8/24 신설]: Brier·방향적중이 *"맞췄나"* 를 본다면 이건 *"맞췄을 때 얼마나 벌고 틀렸을 때 얼마나 잃나"* 를 본다.
  ...
  **★[8/24] 집계를 보면 시점부터 가른다.** 손익비 0.81을 근거로 *"룰4가 조기 익절을 제도화했다"* 는
  가설을 세웠다가 **시점 분해에서 반증**됐다 — 그 40건은 전부 **룰4(7/2 신설)보다 먼저**인 데스크 이전
  거래였고, 룰4 이후 5건은 평균이익 **+23.0%**(이전 +14.9%)로 오히려 개선 방향이었다(d142 종결).
  8/5 룰검증 절차(①집계 → ②에피소드 분해 → ③횡단면)의 **②에서 기각된 사례**다.
  **재검증 트리거(d143)**: 룰4 이후 실현이 **20건**에 도달하면 익절 룰 우열을 재판정한다.
...
- **star_validate** [8/7 신설]: `score_calls`는 **콜 단위**로 집계해 유효표본을 46배 부풀린다(같은 종목이 매일 1표씩).
  ...
  **8/7 판정 = 🟡 방향 견고·유의성 없음** — ⭐5<⭐2 역전이 보정에서 살아남았으나 CI가 전부 겹쳐 구분 불가.
  ...
  ★**[9/14 개정 — 두 축 신설, 그리고 판정이 뒤집혔다]** 5회 연속 🟡이던 '역전'의 상당분이
  **우리 측정기가 만든 것**이었다. 두 결함을 각각 고쳤다:
    · **`--with-watch`(표본)** — 매 보고서 워치 22종목에 ⭐를 매기면서 원장엔 **한 건도** 안 넣고 있었다
      (`score_calls` 백필 소스 = stocks.json = 보유만). 소급 회수 **887콜·종목 23개**,
      독립 단위 **16 → 37(2.31배)**, 별점 버킷당 종목 3.2 → 7.4. ⇒ 원장화 = `watch_calls.py`.
    · **`--beta-adj`(측정)** — 알파가 `fr − br`(단순 차분)이라 베타를 조정하지 않았다. ⭐5 평균 베타
      **1.98** vs ⭐2 **1.09**(별점↔베타 r=+0.35) — 벤치가 빠지는 구간에서 고베타는 품질과 무관하게
      음알파가 찍힌다. 그래서 레짐이 부호를 뒤집었다(폭락기 -7.06%p ↔ 반등기 +2.39%p).
      젠센 알파로 재면 역전 폭이 **절반**으로 준다(20일 -4.26 → -2.15%p).
  **9/14 실측 귀속**: 베타조정 단독 🟡 유지 / **워치 병합 단독으로 판정이 🟢로 전환**(주 효과는 표본).
  둘 다 적용 시 ①집계 3/3→**1/3** · ②ⓑ 3/3→**1/3** · ③b LOO 2/3→**1/3**, 그리고 LOO가 **15개 종목
  제외 전부에서 부호 유지** = 8/29 이후 최대 취약점이던 *"NAVER 하나 빼면 뒤집힌다"*가 해소됐다.
  ⇒ 판정 🟡'방향 견고' → 🟢**'역전이 보정에서 무너졌다'**.
  ⚠️ **그러나 CI는 여전히 0/3이다 — 무판정이다.** 역전을 못 세운 것이지 **별점의 예측력을 입증한 게 아니다.**
     "별점이 맞는 걸로 밝혀졌다"고 쓰지 말 것.
  ⚠️ **병합이 기본값이 아닌 이유**: 워치 별점이 보유만큼 정성껏 매겨졌다는 보장이 없다(노이즈 희석 가능성).
     **두 줄을 나란히** 출력해 갈리는지 본다 — 같으면 그게 8/5 절차 ③(횡단면 재현), 갈리면 그 자체가 산출물이다.
  ⚠️ 워치는 과거 ⭐ 표기가 `⭐N` 단독이라 **스코어가 3%만 동반**된다 → **연속축(④)은 워치로 못 늘린다.**
- **target_reset** [9/9 신설 — target_score의 짝]: 채점기가 *"여력을 크게 부를수록 그만큼 빗나갔다"*(+50%↑ 목표 편향 **+52.6%p**)와 *"밴드 적중률 0~6%"*를 가리켰는데, **고칠 도구가 없었다** — 채점만 하고 끝났다.
  ...
  ★**[9/14] 자기교정 루프 연결 — 낙관계수가 상수였다.** 0.33은 8/8 실측을 **사람이 읽고 손으로 박은 값**이라
  편향이 변해도 안 움직였고, **종목별 편차를 통째로 무시**했다(9/14 실측: 삼성전자 **+64.0%p** ·
  ORCL +58.7 ↔ AAPL **−1.3** · ANET +1.7 — 65%p 벌어지는데 전 종목 같은 0.33).
  이제 `target_score`의 **종목별 실측 편향을 직접 읽어** 중앙값 대비 편차만큼 계수를 움직인다
  (λ=0.5·최소 20콜·클램프 [0.10, 0.65]). 실측 예: 삼성전자 0.30→**0.45** · ORCL 0.38→**0.50** ·
  AAPL 0.41→**0.23** · MSFT 0.53→0.47.
  **왜 절대값이 아니라 중앙값 대비 편차인가** — 12M 목표를 20일로 채점하는 **지평 불일치**는 전 종목 공통이고
  그 공통분은 기본계수가 이미 먹었다. `k = 1 − 실현/내재`처럼 절대값을 쓰면 k가 1에 붙어 **목표가가 현재가로 붕괴**한다.
  ⚠️ **기본 OFF(`--calibrated`로만 적용)** — 목표가는 매수존·트림 판단에 들어가므로 **적용은 정훈 승인 사항**이다.
     단 비교표는 **항상 출력**한다(안 보이면 승인 여부를 물을 수조차 없다).
...
- **multiple_backtest** [8/8 신설 — 정훈 지적 "데이터 쌓이길 기다리지 말고 자료를 구해서"]:
  ...
  ★**[8/22 신설 — 이 도구를 부르는 트리거]** `score_calls`의 **목표가 하단 터치율**이 낮으면
  이 도구로 배수 밴드를 역산한다. **8/22 실측 23%(162/718)** — 우리가 부른 목표의 최소치조차
  4번 중 1번만 닿는다. 목표가 = 선행 EPS × 배수이고 **EPS는 시장에서 받아오므로 안 맞는 건 배수**다.
  ⇒ 주간 R3에서 **하단 터치율과 역사적 정당 구간을 나란히 보고**한다.
  ⚠️ 배수 밴드 변경 = 목표가 변경 = **정훈 승인 사항**. R3는 괴리 보고까지만.
...
1. **기계 점검**: 위 명령이 레이어별 존재·신선도를 FAIL/WARN으로 낸다. FAIL은 그 자리에서 해소.
   **★[8/24 신설] 가드·배선도 같은 층위의 결손이다** — 도구가 늘수록 *"만들었는데 안 불린다 /
   초록불인데 안 잡는다"* 가 쌓인다. 8/23 `check_repealed_rules`가 **7주간 초록불**이었고,
   8/24 `trades.py --realized`(손익비)는 **자동화해놓고 아무 데스크도 안 읽고 있었다**.
   · 가드 커버리지가 안 늘었으면 → **사고 이력이 있는 가드부터** 픽스처 추가(`docs/dev_workflow.md` §1b).
     ⚠️ 숫자를 채우는 게 목적이 아니다 — 억지 픽스처는 통과만 늘리고 사각은 그대로다.
   · 기능 미배선이 늘었으면 → **새로 만든 기능이 판단에 안 흘러가고 있다는 뜻**이다.
