...
> ⚠️ **[8/29 명문화] 별점·스코어는 품질 등급이지 단기 수익률 예측이 아니다.** 실측에서 스코어↔forward알파
> 상관은 3개 지평 전부 음수였고(횡단면 CI가 0을 포함 = 판정 불가), **"⭐5니까 오른다"로 쓰면 안 된다.**
> 진입·청산 타이밍은 매수존·목표가·여력이, 전망의 방향은 `eps_revisions.py`가, 기술신호는 `signal_score.py`가
> 각각 담당한다. 별점은 **무엇을 들고 갈지**를 가르지 **언제 살지**를 가르지 않는다.
