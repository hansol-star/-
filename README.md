...
.claude/
  ...
  skills/
    portfolio-desk/
      ...
      scripts/hunter_score.py      경제사냥꾼 트랙레코드 채점 (정정률·검증률)
      scripts/fundamentals.py      FMP 미국주 펀더멘털 (스코어 하드넘버, FMP_API_KEY)
      scripts/earnings.py          보유·워치 실적 발표일 추적
      scripts/score_calls.py       콜 캘리브레이션 후행채점 (calls_log.jsonl + Brier)
      scripts/decisions.py         결정 메모리 기계 인덱스 (decisions.jsonl)
      scripts/build_app_data.py    portfolio+stocks+시세 → app/data.js 빌드
      scripts/validate_report.py   완료검증 게이트 (보유16·풀표·별점↔스코어·정본 stale)
...
**종목을 누르면** 현재가·평가손익·목표가·매수존·트림·⭐별점·**0~100 정량 스코어**·최근 이슈(날짜·[검증/정정/미확인])가 상세로 펼쳐진다.
...
data/app/
  stocks.json    종목별 분석 정본 (별점·스코어·목표·매수존·트림·이슈) — 보고서가 갱신
...
**📲 아이폰에 "앱"으로 설치 (PWA — 앱스토어 불필요)**
앱은 설치형 PWA(매니페스트·서비스워커·아이콘·오프라인 캐시)다. 앱스토어/개발자계정 없이 홈 화면에 깐다.
1. 레포 **Settings > Pages > Source = "GitHub Actions"** (1회) → `deploy-app.yml`이 main 푸시마다 자동 배포.
2. 아이폰 **사파리**로 배포 URL(`https://hansol-star.github.io/-/`) 열기.
3. 공유 버튼 **⬆️ → "홈 화면에 추가"** → 자체 아이콘·전체화면(주소창 없음)으로 앱처럼 실행, 오프라인에서도 마지막 데이터 표시.
> 아이콘은 `app/make_icons.py`가 stdlib만으로 생성(`app/icons/`). 디자인 바꾸려면 그 스크립트 수정 후 재실행.

로컬에서 바로 보려면 `app/index.html`을 브라우저로 열어도 된다(`data.js` 인라인이라 파일만으로 동작).

**매일 자동**: 일일 보고서는 **Claude Code Routines(구독·무인)**가 생성한다(프롬프트 정본 = `docs/routines.md` R1~R4). 시세는 `refresh-prices.yml`(평일 17:25·05:30 KST, 키0)이 keyless로 `app/data.js`·일별 스냅샷을 갱신하고, `app/**`가 main에 푸시되면 `deploy-app.yml`이 앱을 재배포한다.
> `daily-report.yml`은 **수동(workflow_dispatch)·비상 백업**(Anthropic API 종량제, `ANTHROPIC_API_KEY` 시크릿 필요) — schedule 비활성. Routines 장애 시 Routines-독립 폴백.
