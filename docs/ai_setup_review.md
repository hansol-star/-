...
**2026-07-26 추가 라운드 (B6e~g — 정훈 질문 검증 중 발견한 결함 3건)**: 경제사냥꾼 "상승장" 논지를 실측 검증하다 **우리 쪽 데이터 결함**이 드러나 함께 수정 —
- **B6e — `garch.py` 최신 봉 누락(안전핀 판정 직결 실사고)**: `load_closes`가 history_backfill 캐시만 읽어, 캐시가 밀리면 **최근 거래일이 통째로 빠진 채** GARCH를 적합하고 있었다. 실측 = ^KS11 캐시 마지막이 7/22(6,797.70)라 **7/24 -5.72% 급락(6,690.62)을 못 본 상태로** vol_sizing이 안전핀·폭풍 %ile을 판정 중(같은 시각 vol_gauge는 라이브라 정상). → 캐시 뒤에 **캐시 마지막 날짜보다 새로운 Yahoo 봉만 top-up**(깊은 이력 유지 + 최신성 확보, 네트워크 실패 시 캐시 그대로).
- **B6f — 폭풍 %ile 기준창 불일치**: 룰 문서(CLAUDE.md·crash_tf §5b)는 폭풍 점수를 "**최근 1년** 백분위"로 정의하는데 garch는 **상장 이래 전 이력**(^KS11 7,290봉≈30년) 대비로 재고 있었다 → 같은 시장을 vol_gauge 86%ile / garch 99%ile로 다르게 불러 **§5b(≥90 동결 vs <90 극소창) 판정이 스크립트마다 갈림**. 적합은 전 이력 유지하되 **랭킹 창만 252일로 정렬**(전 이력 값은 `pct_rank_all`로 병기). 교정 후 = 선행 94%ile / 후행 86.1%ile.
- **B6g — §5b 이중 척도 교차확인**: 선행(GARCH 예측)과 후행(RV20 실현)은 같은 날도 갈릴 수 있다(94 vs 86). §5b는 **매수를 허용하는** 게이트이므로 보수적으로 — **둘 다 90 미만일 때만 극소창**, 하나라도 90 이상이면 전면 동결로 vol_sizing을 명시화하고 두 값을 헤드라인에 함께 출력. (기존 대비 느슨해지는 방향의 변경 없음.)
- **B6h — 실적발표일 확정 오버라이드**: Yahoo calendarEvents가 국내 종목 발표일을 하루씩 어긋나게 준다(실측: SK하이닉스 Yahoo 7/28 vs 공식 **7/29 09:00**, 삼성전자 Yahoo 7/29 vs 공식 **7/30 10:00**). 발표일이 밀리면 폰창 대응 계획이 통째로 어긋나므로 `data/app/earnings_overrides.json`(IR 확인분) 레이어를 신설해 event_calendar가 덮어쓰고, Yahoo 값은 `추정(Yahoo)`으로 표기. ※이 오류는 **경제사냥꾼 영상이 우리 캘린더보다 정확**해서 잡혔다 — 채널 견제만이 아니라 채널로부터 배우는 경로도 살아있다는 사례.
...
**2026-08-24 라운드 (B7 — 정훈이 가져온 인스타 추천 리포 10개 검토)**: 전수 실측 결과 **신규 도입 0건**
(6개가 주문 집행기 = 조회전용 룰 충돌 · 3개가 크립토 전용 · 1개는 2026-05 아카이브된 죽은 리포).
게시물의 *"전부 무료 오픈소스"* 는 [정정] — VectorBT는 Commons Clause(비-OSI), Backtrader·Freqtrade는 GPL 전염성.
**검토가 드러낸 우리 구멍 2개가 본 수확**: ①**손익비·기대값을 재는 스크립트가 83개 중 0개**였는데
우리 실제 실패 유형이 정확히 손익비다(미국주 승률 72.5% / 손익비 0.81) → `trades.py _payoff()` 신설로 해소
②**룩어헤드를 주장하는 파일 5개, 검사 장치 0개** — TradingAgents가 같은 자리에서 넘어져(#1115 필터 미실행)
회귀 테스트로 고정한 반면 우리는 코드 테스트 0개(제안·대기). 상세 = `docs/research/repo_review_2026-08-24.md`.
