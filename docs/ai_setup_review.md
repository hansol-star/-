...
Anthropic: 멀티에이전트는 단일대비 **토큰 ~15배**, 가치 높고 병렬·특화일 때만 정당화. ~~우리는 8데스크 전부 opus 4.8~~ **[7/4 개정: 지역2+매크로+리스크+리서치 5개=sonnet / 섹터 3개=opus — CLAUDE.md 데스크별 모델 배분]**.
...
- ~~모델 다운그레이드(Sonnet)는 여전히 기각(결정 d2 — 품질 우선)~~ **[7/4 부분 개정]**: 스크립트 정리·룰체크·태깅 위주 5개 데스크는 sonnet, 별점·스코어 직결 섹터 3개만 opus — d2의 '품질 우선'은 섹터축에 유지하고 비용은 지역·기능축에서 절감. '언제 풀팬아웃하나'(위 분기)와 함께 이중 규율.
