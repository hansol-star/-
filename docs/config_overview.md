...
## 2. 서브에이전트 데스크 (`.claude/agents/`) — 모델 배분 [7/15 개정]: 8개 전부 sonnet(=Sonnet 5) — 품질 저하 시 섹터 3개 opus 롤백 조건부
...
- **보고서**: `docs/reports/report_v{N}_{날짜}.md` (정본 = `docs/reports/`에서 가장 높은 `report_v*` — 현재 **v100**·2026-09-24). 끝에 STATE SNAPSHOT 블록. ※이 버전 토큰은 `validate_report.py`가 stale 검사.
