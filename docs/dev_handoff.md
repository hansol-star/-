...
## ⏳ 정훈 결정 대기 (구조적 — 코드는 준비됨)
- **[vol_gauge] 폭풍 점수 → 트랜치 스케일 매핑 승인**: `vol_gauge.py` 측정 도구는 완성·검증(아래). 안전핀을 "7,500 이진 동결"에서 "변동성 백분위로 트랜치 연속 스케일"로 바꾸는 **매핑은 안전핀 룰 변경=구조적 결정** → 정훈이 매핑(온건/공격/기록만)을 고른 뒤에만 crash_tf.md에 배선. 승인 전까지 도구는 '상황판 참고 수치'로만.
