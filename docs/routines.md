...
- 🔒 **[8/6] R1·R2·R3 프롬프트는 에이전트가 못 고친다** — `http_api`로 생성된 트리거라 `update_trigger`가 거부한다(실측 8/6). ⇒ **루틴 동작을 바꾸려면 코드/스킬 쪽에 심어야 한다.** 프롬프트 문구에 의존하는 방어는 무효. 정훈이 직접 고칠 때만 프롬프트 수정 가능.
  **★[8/24 실측 정정] R4a·R4b는 `created_via: meta_mcp`라 수정 가능하다** — 제약은 R1·R2·R3(http_api)에만 해당한다.
  다만 R4는 '보고서 완주 파수꾼'이라 고칠 내용이 거의 없다.
  **★[8/24 확인] 이 제약은 실무에서 문제가 되지 않았다** — 8/24 변경분(손익비·가드 3종)이
  **프롬프트를 한 줄도 안 고치고 전부 반영됐다**: R2는 `risk-desk.md`(데스크 지시문)를 읽고,
  R3는 `self-review/SKILL.md`를 읽기 때문이다. ⇒ **루틴 최신화의 정석 = 스킬·에이전트 파일을 고친다.**
...
| R4a | 토큰-리셋 재시도 ① | 평일 **20:00** | `0 11 * * 1-5` | **push ON**(재시도로 냈을 때 알림) | 위 R4 코드블록 |
| R4b | 토큰-리셋 재시도 ② | 평일 21:15 | `15 12 * * 1-5` | **push ON**(무인 완주 알림) | 위 R4 코드블록 |

> 프롬프트 정본 = 위 각 루틴의 코드블록. 시간·프롬프트 변경 시 이 문서를 먼저 고치고 트리거를 재등록한다.
> **R4a·R4b는 프롬프트가 거의 동일**(R4a엔 8/27 개정 배경 + '또 막히면 억지 축약본 내지 말고 R4b에 넘겨라' 항목이 추가됨) — cron을 다르게 두 개 등록. 둘 다 이미 보고서가 났으면 거의 0토큰으로 즉시 끝나므로 헛돌아도 무해.
