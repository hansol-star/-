...
- **[8/22 시범] 화면 교차검증**: 자막이 구체 수치·목표가·차트를 주장하는데 웹서치가 갈리거나 안 잡히면
  `yt_frames.py <ID> --at <시각>`으로 그 구간 스토리보드 1장을 받아 Read로 판독한다.
  **상한 = 1회 실행당 최대 2편·편당 1시트**(시트 1장 ≈1,100~1,600토큰). 자막이 끝내 실패한 영상은
  화면으로 주제·종목만 파악하고 "화면만 확인"으로 표기(분석 금지). 판독분은 [화면확인] 태그.
  ⚠️ 160×90이라 차트 축·표 세부 숫자는 안 읽힌다 — 못 읽으면 "화면 확인 불가"로 쓴다.
  **2주 시범 후 R3에서 값어치 판정** → 실제로 [정정]을 잡아냈으면 유지·확대, 아니면 조건을 좁히거나 폐기.
- 정본 기록(메인 보고서가 읽을 캐시):
