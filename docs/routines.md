...
- **[8/22 상시 · 정훈 승인] 화면 교차검증 — 롱폼 한정**: `yt_frames.py <ID> --sheets 2`로
  스토리보드를 받아 Read로 판독한다. **⚠️쇼츠는 판독 불가**(타일 50×90) — 도구가 자동 경고하며,
  경고가 뜨면 즉시 중단하고 자막으로 갈음한다(채널 일일 업로드 상당수가 쇼츠다).
  **표적** = ①[정정] 이력 있는 롱폼 ②수치·차트 주장인데 웹서치가 갈리는 롱폼 ③자막 실패 롱폼.
  **상한 = 1회 실행당 롱폼 2편·편당 2시트.** 판독분은 [화면확인] 태그.
  ⚠️ 차트 축·표 세부는 안 읽힌다 — 못 읽으면 "화면 확인 불가"로 쓴다.
...
- 🩺 **[8/12 신설] API 생존 점검 = `api_health.py`** — 외부 소스 11종 실호출.
...
- **[8/24 신설] 손익비 추이 + 가드·배선 주간 점검** (self-review §0·§7에 심어둠 — 프롬프트 수정 불요):
  ...
  ② `guard_selftest.py --coverage` = 음성테스트 없는 가드(= 초록불 진위 미확인). 현재 3/29.
