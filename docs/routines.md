...
- 🔒 **[8/6] R1·R2·R3·R4 프롬프트는 에이전트가 못 고친다** — `http_api`로 생성된 트리거라 `update_trigger`가 거부한다(실측 8/6). ⇒ **루틴 동작을 바꾸려면 코드/스킬 쪽에 심어야 한다.** 프롬프트 문구에 의존하는 방어는 무효. 정훈이 직접 고칠 때만 프롬프트 수정 가능.
